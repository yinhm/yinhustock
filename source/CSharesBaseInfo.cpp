...
CSharesBaseInfo::CSharesBaseInfo()//基本资料数据结构类实现
{
    m_hFile=NULL;
	m_hFileMap=NULL;
	m_pbData=NULL;
...
BOOL CSharesBaseInfo::InitBaseInfoData(CString Path)//基本资料数据结构类实现
{
...
BOOL CSharesBaseInfo::InitBaseInfoDataExist()// NOTE: the ClassWizard will add member functions here
{
	BYTE *temp;
	m_hFile=CreateFile(g_baseinfo,GENERIC_READ|GENERIC_WRITE,
		FILE_SHARE_READ|FILE_SHARE_WRITE,
	...
	m_BaseFileHead=(BASEINFOHEAD *)m_pbData;
	if(m_BaseFileHead->filetitle!=12345678 && m_BaseFileHead->filetitle!=55555555)// NOTE: the ClassWizard will add member functions here
	{
	   UnmapViewOfFile(m_pbData);
	   CloseHandle(m_hFile);
	   CloseHandle(m_hFileMap);
