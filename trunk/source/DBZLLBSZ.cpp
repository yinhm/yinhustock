// DBZLLBSZ.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CDBZLLBSZ, CDaoRecordset)

CDBZLLBSZ::CDBZLLBSZ(CDaoDatabase* pdb)
	: CDaoRecordset(pdb)
{
	//{{AFX_FIELD_INIT(CDBZLLBSZ)
	m_index = 0;
	m_symbol = _T("");
	m_name = _T("");
	m_time = 0;
...
CString CDBZLLBSZ::GetDefaultDBName()
{
	return _T("C:\\Program Files\\Microsoft Visual Studio\\MyProjects\\vwbase\\历史资料列表.mdb");
}

CString CDBZLLBSZ::GetDefaultSQL()
{
	return _T("[上市公司资料列表-深圳]");
}

void CDBZLLBSZ::DoFieldExchange(CDaoFieldExchange* pFX)
{
	//{{AFX_FIELD_MAP(CDBZLLBSZ)
...
#ifdef _DEBUG
void CDBZLLBSZ::AssertValid() const
{
	CDaoRecordset::AssertValid();
}

void CDBZLLBSZ::Dump(CDumpContext& dc) const
{
