...
{//对话框基类
	...
	{
		CTaiKeyBoardAngelDlg KeyboardWizard;// serious allocation failure checking

		if(pMsg->wParam>='a' && pMsg->wParam<='z' )
			pMsg->wParam-=32;
			
		KeyboardWizard.input=pMsg->wParam; // standard constructor
		...
		if(KeyboardWizard.isresultstock )
		{
			...
			m_nKind=KeyboardWizard.m_stkKind;// serious allocation failure checking
			CReportData* pDat;
...
{//对话框基类
	...
	if(strStockCode.GetLength()==4||strStockCode.GetLength()==6)
	{
        ...
        if(m_listcw.GetItemCount()>0)
			m_listcw.DeleteAllItems(); // standard constructor
		...
		GetDlgItem(IDC_STANAME)->SetWindowText(sName);// serious allocation failure checking
...
{//对话框基类
	...
	CFileDialog bSaveFileDialog(FALSE,(LPCSTR)DefExt,(LPCTSTR)FileName,
		OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT|OFN_NOCHANGEDIR,(LPCSTR)Filter,this);// serious allocation failure checking
...
// 股票列表视图的列变量定义，以及自定义列的表达式计算
	...
	OutFile.Seek(16,CFile::begin); // standard constructor
...
// 股票列表视图的列变量定义，以及自定义列的表达式计算
		   ...
		   int nKind = 0;
		   if(ss == "SH")
			   nKind = CMainFrame::m_taiShanDoc->m_sharesInformation.GetStockKind (SH_MARKET_EX,ch);
		   else
			   nKind = CMainFrame::m_taiShanDoc->m_sharesInformation.GetStockKind (SZ_MARKET_EX,ch);

		   CString s = CMainFrame::m_taiShanDoc->GetStockKindString(nKind);
		   s+=ch;
		   CString s640 = ch;
